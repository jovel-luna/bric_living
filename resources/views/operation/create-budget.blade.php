@extends('layouts.app', ['pageSlug' => 'create-budget-page'])
...
<div class="content-header">
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <form action="{{ route('store.budget', $data->id)}}" method="POST">
                @csrf
                <div class="card card-secondary shadow mb-4 p-0">
                    <div class="card-header py-2">
                        <h6 class="m-0 font-weight-bold">{{ __('Budget Details') }}</h6>
                    </div>
                    <div class="card-body">
                        <!-- Budget -->
                                ...
                                <div class="mb-3">
                                    <div class="row">
                                        <div class="col-md-3">
                                            <!-- Budget Year -->
                                            <div class="mb-3">
                                                <label for="total_opex_budget" class="col-form-label">{{ __('Budget Year') }}<span class="isRequired"> * </span></label>
                                                <input type="text" class="form-control @error('budget_year') is-invalid @enderror" name="budget_year" id="budget_year" value="{{ old('budget_year') }}" autocomplete="budget_year" placeholder="YYYY">
                                                @error('budget_year')
        ...
        $(document).ready( function () {
            ...
            $("form").submit(function(e){
                ...
                if(validateStatus){
                    ...
                    jQuery.ajax({
                        url: "{{ route('store.budget', $data->id) }}",
                        method: 'post',
                        data: {
                            formData: formData,
                        },
            ...
            function validateFields(){
                if ($("input[name='budget_year']").val() != '' && $("input[name='budget_year']").val() != null) {
                    return true;
                }else{
                    if ($("input[name='budget_year']").val() == "") { $("input[name='budget_year']").addClass("is-invalid") }
                    Swal.fire({
            ...
            $(document).on('change', '#datepicker', function(e){
                $("input[name='budget_year']").removeClass("is-invalid")
            });
            // OPERATIONS
