        ...
        <div class="container-fluid">
            <div class="row justify-content-center">
                ...
                <div id="lettings" class="col-md-12">
                    ...
                    <table id="letting-table" class="table table-bordered letting-table property-list-table m-0"
                        style="width:100%;" autoCompletet>
                        <thead>
                            <tr>
                                <th>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#clear-filter').on('click', function() {
                    $('select#property_phase, select#entity, select#city, select#area, select#no_bric_beds, select#status, select#property_contract_status')
                ...
                $('#filter-view').on('click', function() {
                    ...
                    letting(property_phase, entity, city, area, no_bric_beds, status, postcode, address,
                        show_limit, $('#searching').val(), property_contract_status);
                });
                ...
                $('#searching').keyup(function() {
                    ...
                    getArrayData();
                    $('#letting-table').DataTable().destroy();
                    letting($('select#property_phase').val(), $('select#entity').val(), $('select#city').val(),
                        $('select#area').val(), $('select#no_bric_beds').val(), $('select#status').val(),
                        postcode, address, show_limit, search);
                });
                ...
                function letting(property_phase = '', entity = '', city = '', area = '', no_bric_beds = '', status = '',
                    postcode = '', address = '', showlimit = '', search = '', property_contract_status = '') {
                    ...
                    var lettingTable = $('#letting-table').DataTable({
                        ...
                        columns: [{
                            ...
                            },
                            {
                                ...
                                render: function(data, type, row) {
                                    if (cua === true || cua === 'true') {
                                        return '<input id="date_available_' + row.DT_RowIndex +
                                            '" type="text" class="form-control has-datepicker" name="date_available" value="' +
                                            (row.projected_completion_date == null || row
                                                .projected_completion_date == '' ? '' : row
                                                .projected_completion_date) +
                                            '" placeholder="dd-mm-yyyy" style="height: 30px; padding: 0;">';
                                    } else {
                                        return row.projected_completion_date != 'null' || row
                                            .projected_completion_date != '' ? "" : row
                                            .projected_completion_date;
                                    }
                                }
                            ...
                            {
                                data: 'property_contract_status',
                                name: 'property_contract_status',
                                orderable: true,
                                render: function(data, type, row) {
                                    if (cua === true || cua === 'true') {
                                        var csList = [
                                        ...
                                        $.each(csList, function(cs, csVal) {
                                            if (csVal == row.property_contract_status) {
                                                tempPSoption += '<option value="' + csVal +
                                                    '" selected>' + csVal + '</option>';
                                            } else {
                                                tempPSoption += '<option value="' + csVal +
                                                    '">' + csVal + '</option>';
                                            }
                                        });
                                        var tempPSselect = '<select id="contract_status_' + row
                                            .DT_RowIndex +
                                            '" name="contract_status" class="form-control contract_status" style="height: 30px; padding: 0;"><option value="">Please Select</option>' +
                                            tempPSoption + '</select>';
                                        return tempPSselect;
                                    } else {
                                        return row.letting_status_name ? row.letting_status_name :
                                        'N/A';
                                    }
                                }
                            },
                                ...
                                render: function(data, type, row) {
                                    if (cua === true || cua === 'true') {
                                        var tempBedBath =
                                            ...
                                            '<input id="bric_bed" name="bric_bed" type="text" class="form-control" style="padding: 0; width: 25px; height: 30px;" value="' +
                                            row.no_bric_beds + '">' +
                                            '&nbsp;/&nbsp;' +
                                            '<input id="bric_bath" name="bric_bath" type="text" class="form-control" style="padding: 0; width: 25px; height: 30px;" value="' +
                                            row.no_bric_bathrooms + '">' +
                                            '</div>';
                                        return tempBedBath;
                                    } else {
                                ...
                                render: function(data, type, row) {
                                    if (cua === true || cua === 'true') {
                                        ...
                                        $.each(vList, function(xvl, xvlv) {
                                            if (xvlv == row.version) {
                                                tempVoption += '<option value="' + xvlv +
                                                    '" selected>' + xvlv + '</option>';
                                            } else {
                                                tempVoption += '<option value="' + xvlv + '">' +
                                                    xvlv + '</option>';
                                            }
                                        });
                                        var tempVselect = '<select id="version_' + row.DT_RowIndex +
                                            '" name="version" class="form-control version" style="height: 30px; padding: 0;"><option value="">Please Select</option>' +
                                            tempVoption + '</select>';
                                        return tempVselect;
                                    } else {
                            ...
                            {
                                data: 'target_weekly_rent',
                                name: 'target_weekly_rent',
                                orderable: true,
                                render: function(data, type, row) {
                                    if (cua === true || cua === 'true') {
                                        var twr = row.target_weekly_rent ? row.target_weekly_rent : '';
                                        var temptwr = '<input type="text" id="target_weekly_rent_' + row
                                            .DT_RowIndex +
                                            '" name="target_weekly_rent" onkeyup="formatNumber(this.id)" class="form-control mx-auto target_weekly_rent" value="' +
                                            twr +
                                            '" style="padding: 0; height: 30px;" placeholder="Enter amount">';
                                        return temptwr;
                                    } else {
                                ...
                                render: function(data, type, row) {
                                    if (cua === true || cua === 'true') {
                                        ...
                                        var tempNotes = '<div>' +
                                            ...
                                            '" class="btn btn-success btn-xs mb-1 add-notes" data-toggle="modal" data-target="#addNotes">' +
                                            ...
                                            '<div>' +
                                            '<a href="#" type="button" data-id="' + row.id +
                                            '" class="btn btn-info btn-xs view-notes">' +
                            ...
                            },
                            {
                                ...
                                render: function(data, type, row) {
                                    return '<div class="action-btn d-flex gap-1 justify-content-center">' +
                                        '<a href="' + baseUrl + '/' + 'property/details/' + row.id +
                                        '" id="view-txt-btn" class="view" title="View"><i class="view icon fa-regular fa-eye" style="color: #005eff;"></i></a>' +
                                        '</div>';
                                }
                            },
                        ]
                ...
                $(document).on('click', '.remove-btn', function() {
                    $(this).parent('.list-items').fadeOut(300, function() {
                        $(this).remove();
                    });
                });
                                ...
                                $('table tbody tr').each(function(e) {
                                    if ($(this).find('.selectItem').is(':checked')) {
                                        ...
                                        updateData.push({
                                            'id': parseInt(letid),
                                            'available': $(this).find(
                                                'input[name="date_available"]').val(),
                                            'contract_status': $(this).find('.contract_status')
                                                .children("option:selected").val(),
                                            'bric_bed': $(this).find('#bric_bed').val(),
                                            'bric_bath': $(this).find('#bric_bath').val(),
                                            'version': $(this).find('.version').children(
                                                "option:selected").val(),
                                            'target_weekly_rent': $(this).find(
                                                'input[name="target_weekly_rent"]').val(),
                                        });
                                    }
                                });
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response['status'] == 1) {
                                ...
                                var name = response['data']['first_name'] + ' ' + (response['data'][
                                    'middle_name'
                                ] ? response['data']['middle_name'] : '') + ' ' + response[
                                    'data']['last_name'];
                                var has_update = moment(response['data']['created_at']).isSame(
                                    response['data']['updated_at']);
                                var date_time_created_format = moment(response['data'][
                                    'created_at']).format('MMMM Do YYYY, h:mm a');
                                var date_time_updated_format = moment(response['data'][
                                    'updated_at']).format('MMMM Do YYYY, h:mm a');
                                var date_time_diff_created_format = moment(response['data'][
                                    'created_at'
                                ]).startOf('hour').fromNow();
                                var date_time_diff_updated_format = moment(response['data'][
                                    'updated_at'
                                ]).startOf('hour').fromNow();

                                var note_date = has_update ? date_time_created_format :
                                    date_time_updated_format;
                                var note_label = has_update ? 'Created by: ' : 'Updated by: ';
                                var note_diff = has_update ? date_time_diff_created_format :
                                    date_time_diff_updated_format;
                                ...
                                parent_row.find('.note_description').text(response['data'][
                                    'description'
                                ]);
                            ...
                            if (response['status'] == 1) {
                                ...
                                if (response['data'].length != 0) {
                                    $.each(response['data'], function(x, xVal) {
                                        var has_update = moment(xVal['created_at']).isSame(
                                            xVal['updated_at']);
                                        var name = xVal['first_name'] + ' ' + (xVal[
                                                'middle_name'] ? xVal['middle_name'] :
                                            '') + ' ' + xVal['last_name'];
                                        var date_time_created_format = moment(xVal[
                                            'created_at']).format(
                                            'MMMM Do YYYY, h:mm a');
                                        var date_time_updated_format = moment(xVal[
                                            'updated_at']).format(
                                            'MMMM Do YYYY, h:mm a');
                                        var date_time_diff_format = moment(xVal[
                                            'created_at']).startOf('hour').fromNow();
                                        if (cua === true || cua === 'true') {
                                            accessTmp = '<div class="d-flex gap-1">' +
                                                '<a href="#" data-action="edit" data-id="' +
                                                xVal['id'] +
                                                '" class="edit icon edit-note">' +
                                                ...
                                                '</svg>' +
                                                '</a>' +
                                                '<a href="#" data-action="delete" data-id="' +
                                                xVal['id'] +
                                                '" class="delete icon delete-note">' +
                                        ...
                                        noteTmp += '<li class="note-item mb-2">' +
                                            ...
                                            '<div>' +
                                            '<div>Date: <strong class="note_date">' + (
                                                has_update ? date_time_created_format :
                                                date_time_updated_format) +
                                            '</strong></div>' +
                                            '<div><span class="note_label">' + (has_update ?
                                                'Created' : 'Updated') +
                                            ' by: </span><strong class="note_publisher">' +
                                            name + '</strong></div>' +
                                            '</div>' + (cua === true || cua === 'true' ?
                                                accessTmp : '') + '</div>' +
                                            '<div class="mb-1 note_description">' + xVal[
                                                'description'] + '</div>' +
                                            '<div class="d-flex justify-content-end time-passed"><strong class="note_diff">' +
                                            date_time_diff_format + '</strong></div>' +
                                            '</li>';
                                    });
                                } else {
                                    noteTmp +=
                                        '<li class="no-result"> <h4>No Available Notes</h4> </li>';
                                }
                                ...
                                success: function(response) {
                                    if (response.success == 1) {
                                        ...
                                        parent_item.fadeOut(500, function() {
                                            $(this).remove();
                                            console.log(
                                                "🚀 ~ file: index.blade.php:565 ~ $ ~ children:",
                                                $("#notes-logs").children("li")
                                                .length)
                                            if ($("#notes-logs").children("li")
                                                .length == 0) {
                                                noteTmp =
                                                    '<li class="no-result"> <h4>No Available Notes</h4> </li>';
                                                $("#notes-logs").empty().append(
                                                    noteTmp);
                                            }
                                        });
                                    }
