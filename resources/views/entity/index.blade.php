            ...
            <div class="row">
                ...
                <div class="col-sm-6 d-flex align-items-center justify-content-end gap-4">
                    ...
                    <div class="func-btns d-flex align-items-center">
                        <a href="{{ route('entity.create')}}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
        ...
        $(document).ready( function () {
            ...
            var table = $('#entityTable').DataTable({
                ...
                columns: [
                    ...
                    {data: 'registered_address', name: 'registered_address'},
                    {data: 'no_of_properties', name: 'no_of_properties', orderable: false, searchable: false, render:function(data, type, row){
                        return row.no_of_properties ? row.no_of_properties : 'N/A';
                    }},
                    {data: 'no_bric_beds', name: 'no_bric_beds', orderable: false, searchable: false, render:function(data, type, row){
                        return row.no_bric_beds ? row.no_bric_beds : 'N/A';
                    }},
                    {data: 'pipeline', name: 'pipeline', orderable: false, searchable: false, render:function(data, type, row){
                        return row.pipeline ? row.pipeline : 'N/A';
                    }},
                    {data: 'current_rent_role', name: 'current_rent_role', orderable: false, searchable: false, render:function(data, type, row){
            ...
            function getEntityData(rowID){
                jQuery.ajax({
                    ...
                    success: function(response){
                        if (response) {
                            $('#side-company_registration_number').val(response['data']['company_registration_number']);
                            ...
                            $('#side-financial_year_start_date').val(response['data']['financial_year_start_date']);
                            $('#side-financial_year_end_date').val(response['data']['financial_year_end_date']);
                        }
